 ...
 * - Créer / modifier / supprimer / changer statut : TOUS LES MEMBRES POLICE + Admin
 *   (la Police de Sunagakure gère la justice opérationnelle)
 * ════════════════════════════════════════════════════════════════
 */
...
export default function PlaintesPage() {
  ...
  function openCreate() {
    setEditingId(null);
    setForm({ type: 'Vol', cible: 'Citoyen', statut: 'ouverte' });
    setShowForm(true);
  }
  ...
  async function setStatut(p: Plainte, newStatut: PlainteStatut) {
    try {
      ...
      if (newStatut === 'en_cours' && !updated.agent) updated.agent = CURRENT_USER;
      if (newStatut === 'fermee') updated.agentCloture = CURRENT_USER;
      list[idx] = updated;
      await persistAll(list);
  ...
  async function handlePhotoUpload(file: File) {
    if (!file.type.startsWith('image/')) {
      toast.error("Le fichier n'est pas une image"); return;
    }
    try {
  ...
  return (
    <>
      <Card
        ...
        actions={
          <RequireMembreBranche branche="police">
            <Button onClick={openCreate}><Plus size={14} /> Déposer une plainte</Button>
          </RequireMembreBranche>
        }
      >
        <div className={styles.tabs}>
          <button className={`${styles.tab} ${tab === 'ouverte' ? styles.tabActive : ''}`} onClick={() => setTab('ouverte')}>
            <AlertCircle size={14} /><span>Ouvertes</span>
            <span className={styles.tabCount}>{counts.ouverte}</span>
          </button>
          <button className={`${styles.tab} ${tab === 'en_cours' ? styles.tabActive : ''}`} onClick={() => setTab('en_cours')}>
            <ShieldAlert size={14} /><span>En cours</span>
            <span className={styles.tabCount}>{counts.en_cours}</span>
          </button>
          <button className={`${styles.tab} ${tab === 'fermee' ? styles.tabActive : ''}`} onClick={() => setTab('fermee')}>
            <CheckCircle2 size={14} /><span>Fermées</span>
            <span className={styles.tabCount}>{counts.fermee}</span>
          </button>
          <button className={`${styles.tab} ${tab === 'transmise_tribunal' ? styles.tabActive : ''}`} onClick={() => setTab('transmise_tribunal')}>
            <Scale size={14} /><span>Tribunal</span>
          ...
          <button className={`${styles.tab} ${tab === 'all' ? styles.tabActive : ''}`} onClick={() => setTab('all')}>
            <FileText size={14} /><span>Toutes</span>
        ...
        <div className={styles.searchBox}>
          <Search size={14} />
          <input type="text" placeholder="Rechercher par ref, plaignant, accusé, type…"
        ...
        {loading ? <p className={styles.empty}>Chargement…</p>
          : visiblePlaintes.length === 0 ? (
            <div className={styles.empty}>
              ...
              <p>
                {search ? 'Aucune plainte pour cette recherche.'
                  : tab === 'ouverte' ? 'Aucune plainte ouverte.'
                  : tab === 'en_cours' ? 'Aucune plainte en cours d\'enquête.'
                  : tab === 'fermee' ? 'Aucune plainte fermée.'
                  : tab === 'transmise_tribunal' ? 'Aucune plainte transmise au Tribunal.'
            ...
            <div className={styles.list}>
              {visiblePlaintes.map((p) => (
                <article key={p.id} className={`${styles.plainte} ${styles[`st-${p.statut}`]}`} onClick={() => setViewingId(p.id)}>
        ...
        footer={
          viewing && canEdit && (
            <>
              ...
              <Button variant="outline" onClick={() => openEdit(viewing)}>Modifier</Button>
              <div style={{ flex: 1 }} />
              {viewing.statut === 'ouverte' && (
                <Button onClick={() => setStatut(viewing, 'en_cours')}>Prendre en charge</Button>
              )}
        ...
        {viewing && (
          <div className={styles.viewer}>
            ...
            <div className={styles.viewerGrid}>
              ...
              <Field label="Date des faits" value={viewing.dateFaits ? fmtDateFR(viewing.dateFaits) : '—'} />
              ...
              <Field label="Agent en charge" value={
                viewing.agent
      ...
      <Modal open={showForm} onClose={closeForm}
        title={editingId ? 'Modifier la plainte' : 'Déposer une plainte'} size="lg"
        footer={
          <>
            <Button variant="outline" onClick={closeForm}>Annuler</Button>
            <Button onClick={handleSave}><Save size={14} /> Enregistrer la plainte</Button>
          </>
        }
      >
        <div className={styles.formFields}>
          <div className={styles.row}>
            <label>Plaignant *
              <input type="text" value={form.plaignant ?? ''}
                onChange={(e) => setForm({ ...form, plaignant: e.target.value })}
                autoFocus placeholder="Nom du plaignant" />
            </label>
            <label>Accusé *
              <input type="text" value={form.accuse ?? ''}
          ...
          <div className={styles.row3}>
            <label>Type d&apos;infraction
              <select value={form.type ?? 'Vol'}
                onChange={(e) => setForm({ ...form, type: e.target.value as PlainteType })}>
                {PLAINTE_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </label>
            <label>Cible
              <select value={form.cible ?? 'Citoyen'}
                onChange={(e) => setForm({ ...form, cible: e.target.value as PlainteCible })}>
            ...
            <label>Date des faits
              <input type="date" value={form.dateFaits ?? ''}
          ...
          <label>
            ...
            <div className={styles.uploadZone}>
              {form.photoAccuse ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={form.photoAccuse} alt="Aperçu" className={styles.uploadPreview} />
              ) : (
              ...
              {form.photoAccuse && (
                <button type="button" className={styles.removePhoto}
                  onClick={(e) => { e.preventDefault(); setForm({ ...form, photoAccuse: undefined }); }}>
                  Retirer
                </button>
              )}
            </div>
