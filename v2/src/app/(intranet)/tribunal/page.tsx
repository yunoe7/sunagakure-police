 ...
 *   (décisions de justice = sensibles)
 * ════════════════════════════════════════════════════════════════
 */

import { useMemo, useState } from 'react';
import {
  Plus, Trash2, Save, Folder, Gavel, Scale, Calendar, MapPin,
...
import {
  type Affaire, type Audience, type Jugement,
  type AffaireStatut, type AudienceStatut, type JugementVerdict,
  AFFAIRE_STATUT_LABEL, AUDIENCE_STATUT_LABEL, VERDICT_LABEL,
  nextAffaireRef, fmtDateFR,
} from '@/types/tribunal';
...
export default function TribunalPage() {
  ...
  async function handleSaveJugement() {
    if (!form.titre?.trim()) { toast.error("Le titre / l'affaire est obligatoire"); return; }
    try {
    ...
    <>
      <Card
        ...
        actions={
          canEdit && (
            <Button onClick={() =>
              openCreate(tab === 'affaires' ? 'affaire' : tab === 'audiences' ? 'audience' : 'jugement')
            }>
              <Plus size={14} />
              {tab === 'affaires' ? 'Nouvelle affaire'
                : tab === 'audiences' ? 'Planifier audience'
                : 'Rendre jugement'}
            </Button>
          )
        }
      >
        <div className={styles.tabs}>
          <button className={`${styles.tab} ${tab === 'affaires' ? styles.tabActive : ''}`}
            ...
            <Folder size={14} /><span>Affaires</span>
            <span className={styles.tabCount}>{affaires.length}</span>
          </button>
          <button className={`${styles.tab} ${tab === 'audiences' ? styles.tabActive : ''}`}
            onClick={() => setTab('audiences')}>
            <Gavel size={14} /><span>Audiences</span>
            <span className={styles.tabCount}>{audiences.length}</span>
          </button>
          <button className={`${styles.tab} ${tab === 'jugements' ? styles.tabActive : ''}`}
            onClick={() => setTab('jugements')}>
            <Scale size={14} /><span>Jugements</span>
              ...
              <div className={styles.list}>
                {affairesSorted.map((a) => (
                  <article key={a.id}
                    ...
                    onClick={() => canEdit && openEdit('affaire', a)}
                    ...
                    <div className={styles.itemHeader}>
                      ...
                      {canEdit && (
                        <button className={styles.deleteBtn}
                          onClick={(e) => { e.stopPropagation(); handleDelete('affaire', a); }}
                    ...
                    <div className={styles.itemMeta}>
                      {a.defendeur && (
                        <span><UserIcon size={11} /> Défendeur: <strong>{a.defendeur}</strong></span>
                      )}
                      {a.accusateur && (
                        <span><UserIcon size={11} /> Accusateur: <strong>{a.accusateur}</strong></span>
                      )}
              ...
              <div className={styles.list}>
                {audiencesSorted.map((a) => (
                  <article key={a.id}
                    ...
                    onClick={() => canEdit && openEdit('audience', a)}
                    style={canEdit ? { cursor: 'pointer' } : { cursor: 'default' }}>
                    <div className={styles.itemHeader}>
                      {a.date && (
                      ...
                      {canEdit && (
                        <button className={styles.deleteBtn}
                          onClick={(e) => { e.stopPropagation(); handleDelete('audience', a); }}
                          aria-label="Supprimer">
                          <Trash2 size={13} />
                        </button>
                      )}
                    ...
                    <h3 className={styles.itemTitle}>{a.titre}</h3>
                    <div className={styles.itemMeta}>
                      {a.lieu && <span><MapPin size={11} /> {a.lieu}</span>}
              ...
              <div className={styles.list}>
                {jugementsSorted.map((j) => (
                  <article key={j.id}
                    ...
                    onClick={() => canEdit && openEdit('jugement', j)}
                    style={canEdit ? { cursor: 'pointer' } : { cursor: 'default' }}>
                    <div className={styles.itemHeader}>
                      ...
                      {canEdit && (
                        <button className={styles.deleteBtn}
                          onClick={(e) => { e.stopPropagation(); handleDelete('jugement', j); }}
                          aria-label="Supprimer">
                          <Trash2 size={13} />
                        </button>
                      )}
                    ...
                    <div className={styles.itemMeta}>
                      ...
                      {j.peine && <span className={styles.peineTag}>Peine: {j.peine}</span>}
                    </div>
      ...
      {/* ─── Modale AFFAIRE ─── */}
      <Modal open={editingType === 'affaire'} onClose={closeModal}
        title={editingId ? "Modifier l'affaire" : 'Nouvelle affaire'} size="lg"
        footer={
          <>
            <Button variant="outline" onClick={closeModal}>Annuler</Button>
            <Button onClick={handleSaveAffaire}><Save size={14} /> Enregistrer</Button>
          </>
        }
      >
        <div className={styles.formFields}>
          ...
          <div className={styles.row}>
            <label>Défendeur (accusé)
              <input type="text" value={form.defendeur ?? ''}
                onChange={(e) => setForm({ ...form, defendeur: e.target.value })} />
            </label>
            <label>Accusateur (partie civile)
              <input type="text" value={form.accusateur ?? ''}
          ...
          <div className={styles.row}>
            ...
            <label>Avocat de la défense
              <input type="text" value={form.avocat ?? ''}
                onChange={(e) => setForm({ ...form, avocat: e.target.value })} />
            </label>
          </div>
          <label>Statut
            <select value={form.statut ?? 'instruction'}
      ...
      {/* ─── Modale AUDIENCE ─── */}
      <Modal open={editingType === 'audience'} onClose={closeModal}
        title={editingId ? "Modifier l'audience" : 'Planifier une audience'} size="lg"
        footer={
          <>
            <Button variant="outline" onClick={closeModal}>Annuler</Button>
            <Button onClick={handleSaveAudience}><Save size={14} /> Enregistrer</Button>
          </>
        }
      >
        <div className={styles.formFields}>
          ...
          <div className={styles.row}>
            <label>Affaire associée (optionnel)
          ...
          <label>Statut
            <select value={form.statut ?? 'planifiee'}
      ...
      {/* ─── Modale JUGEMENT ─── */}
      <Modal open={editingType === 'jugement'} onClose={closeModal}
        title={editingId ? 'Modifier le jugement' : 'Rendre un jugement'} size="lg"
        footer={
          <>
            <Button variant="outline" onClick={closeModal}>Annuler</Button>
            <Button onClick={handleSaveJugement}><Save size={14} /> Enregistrer</Button>
          </>
        }
      >
        <div className={styles.formFields}>
          ...
          <div className={styles.row}>
            <label>Verdict *
              <select value={form.verdict ?? 'coupable'}
          ...
          <label>Peine prononcée
            <input type="text" value={form.peine ?? ''}
              onChange={(e) => setForm({ ...form, peine: e.target.value })}
              placeholder="Ex: 6 mois de prison + 5000 ryos d'amende" />
          </label>
          <label>Motifs / Considérants
            <textarea rows={4} value={form.motifs ?? ''}
