 ...
 * ════════════════════════════════════════════════════════════════
 *
 * Stockage Firebase : sunagakure/infractions (TABLEAU, format legacy)
 * Compatible avec l'ancien intranet.
 *
 * Pattern : similaire à Annonces (tableau Firebase).
 * Affichage : groupé par catégorie (mineur → majeur → crime → peine capitale).
 * ════════════════════════════════════════════════════════════════
 */
...
export default function CodePenalPage() {
  ...
  // ─── Liste filtrée + groupée par catégorie ───
  ...
  return (
    <>
      ...
      <Modal
        open={showForm}
        onClose={closeForm}
        title={editingId ? "Modifier l'ordonnance" : 'Nouvelle ordonnance'}
        size="lg"
        footer={
          <>
            ...
            <Button onClick={handleSave}>
              <Save size={14} /> Sceller l&apos;ordonnance
            </Button>
          </>
        }
      >
        <div className={styles.formFields}>
          ...
          <label>
            Catégorie / Gravité
            <select
              value={form.cat ?? 'violet'}
              onChange={(e) =>
                setForm({ ...form, cat: e.target.value as InfractionCat })
              }
            >
              {INFRACTION_CAT_ORDER.map((c) => (
                <option key={c} value={c}>
                  {INFRACTION_CAT_LABEL[c]}
                </option>
              ))}
            </select>
          </label>
          ...
          <div className={styles.row}>
            ...
            <label>
              ...
              <input
                ...
                disabled={form.prison !== 'Oui'}
              />
            </label>
          </div>

          <label>
            Notes / Description légale
            <textarea
              rows={4}
              value={form.notes ?? ''}
